...
frame_name_re = r'frame\.(\d+)\.\w+\.hdf5'

def parse_index(filename: str) -> int | None:
    """
    Use a regular expression to parse out the index from a filename
    """
    match = re.match(frame_name_re, filename)
    if match is None:
        return None
    return match.group(1)


def extract_array(filepath: str) -> h5py.Dataset | None:
    """
    Attempt to extract a dataset array from an hdf5 file.
    """
    if not os.path.exists(filepath):
        print(f"Warning: failed to extract array, file at {filepath} does not exist")
        return None
    ...
    try:
        ...
        print(f"Warning: failed to extract array, {e}")
        return None

    print(f"Failed to extract dataset from {filepath}")
    return None


async def async_download_unzip_transform(url: str):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        async with httpx.AsyncClient() as client:
            ...
            with open(zip_file_path, "wb") as f, tqdm(
                total=total, unit="iB", unit_scale=True, desc="Downloading ZIP"
            ) as progress_bar:
        ...
        mappings = defaultdict(dict)  # map frame number to `image`, `depth`, `normal`
        for root, dirs, files in os.walk(temp_dir):
            if "final" in root and "preview" not in root:
                # process RGBs
                for file in files:
                    if not file.endswith("hdf5") and "color" not in file:
                        continue
                    frame_idx = parse_index(file)
                    if frame_idx is None:
                        print(f"Warning: failed to parse index for {file} in {url}")
                        continue
                    frame_filepath = os.path.join(root, file)
                    mappings[frame_idx]["image"] = extract_array(frame_filepath)

            if "geometry" in root and "preview" not in root:
                # process geometries
                for file in files:
                    ...
                    if "depth" in file:
                        ...
                        if frame_idx is None:
                            ...
                            continue
                        frame_filepath = os.path.join(root, file)
                        mappings[frame_idx]["depth"] = extract_array(frame_filepath)
                    if "normal_cam" in file:
                        frame_idx = parse_index(file)
                        if frame_idx is None:
                            print(f"Warning: failed to parse index for {file} in {url}")
                            continue
                        frame_filepath = os.path.join(root, file)
                        mappings[frame_idx]["normal"] = extract_array(frame_filepath)

    print(f"Finished transforming {len(mappings)} images")

    return mappings


def download_unzip_transform(url: str):
    with tempfile.TemporaryDirectory() as temp_dir:
        zip_file_path = os.path.join(temp_dir, "downloaded.zip")
...
def write_to_disk(mappings: dict, prev_idx: int = 0) -> int:
    """
    Write the mappings to disk. Use previous index to offset current count.
    Previous index should be exclusive, meaning we should start from the prev_idx
    value.
    """
    curr_idx = prev_idx
    for frame, arrays in mappings.items():
        if "image" not in arrays:
            print(f"Malformed array for frame {frame}, missing image")
            continue
        if "depth" not in arrays:
            print(f"Malformed array for frame {frame}, missing depth")
            continue
        if "normal" not in arrays:
            print(f"Malformed array for frame {frame}, missing normal")
            continue

        idx_dir = str(curr_idx).rjust(7, '0')
        os.makedirs(idx_dir, exist_ok=True)

        try:
            np.save(f"{idx_dir}/frame.npy", arrays['image'])
            np.save(f"{idx_dir}/depth.npy", arrays['depth'])
            np.save(f"{idx_dir}/normal.npy", arrays['normal'])
        except Exception as exc:
            print(f"Failed to save arrays for frame {frame}, exception: {exc}")
            continue
...
if __name__ == "__main__":
    import asyncio
    import os
    os.chdir("/data/hypersim")

    # run a producer and a consumer
    mapping_queue = asyncio.Queue(maxsize=10)

    async def producer():
        start = 0  # can override this
        for url in URLS[start:]:
            try:
                mapping = await async_download_unzip_transform(url)
                await mapping_queue.put(mapping)
            except Exception as exc:
                print(f"Encountered exception in downloading {url}: {repr(exc)}")
        await mapping_queue.put(None)

    async def consumer():
        loop = asyncio.get_running_loop()
        curr_idx = 0  # can override this
        while True:
            mapping = await mapping_queue.get()
            if mapping is None:
                print("Finished consuming")
                return
            curr_idx = await loop.run_in_executor(None, write_to_disk, mapping, curr_idx)
            print(f"Image index updated to {curr_idx}")
