...
if __name__ == "__main__":
    ...
    print(
        f"data_size: {data_size}, train_size: {train_size}, val_size: {val_size}, test_size: {test_size}")
    ...
    for e in range(start_epoch, epochs):
        ...
        for i, (j, x, z_gt, n_gt) in enumerate(train_loader):
            ...
            if torch.isnan(depth_loss).any() or torch.isnan(norm_loss).any():
                print(
                    "Encountered nan in model output. Training will fail from here.")
                breakpoint()
            ...
            if (i + 1) % 10 == 0 or i == 0:
                ...
                avg_d_loss = rdl / (i + 1)
                print(
                    f"Epoch {e}, iter {i + 1}/{max_iters} — Loss: {avg_loss:.4f}, Loss Norm: {avg_n_loss:.4f}, Loss Depth: {avg_d_loss:.4f}")
                ...
                # writer.add_scalar(
                #     "param/log_sigma_d",
                #     criterion.log_sigma_d.item(),
                #     i)
                # writer.add_scalar(
                #     "param/log_smg_an",
                #     criterion.log_sigma_n.item(),
                #     i)
